...
- [1 入门指南](#1-入门指南)
  - [1.1 简介](#11-简介)
  - [1.2 安装 Rust](#12-安装-rust)
  - [1.3 Hello World](#13-hello-world)
  - [1.4 cargo](#14-cargo)
- [2 猜数游戏](#2-猜数游戏)
  - [2.1 一次猜测](#21-一次猜测)
  - [2.2 生成神秘数字](#22-生成神秘数字)
- [3 数据类型](#3-数据类型)
  - [3.1 变量与可变性](#31-变量与可变性)
  - [3.2 数据类型](#32-数据类型)
  - [3.3 数据类型：复合类型](#33-数据类型复合类型)
  - [3.4 函数与注释](#34-函数与注释)
  - [3.5 控制流：if else](#35-控制流if-else)
  - [3.6 循环](#36-循环)
- [4 所有权](#4-所有权)
  - [1、所有权存在的原因](#1所有权存在的原因)
  - [2、所有权的规则是什么？](#2所有权的规则是什么)
  - [3、从 String 类型看所有权](#3从-string-类型看所有权)
  - [4、什么是所有权转移？](#4什么是所有权转移)
  - [5、克隆与复制](#5克隆与复制)
  - [> PS：适当的使用所有权，生活更美好。使用克隆和复制，可以不用考虑所有权的转移，不过可能会导致性能问题。](#-ps适当的使用所有权生活更美好使用克隆和复制可以不用考虑所有权的转移不过可能会导致性能问题)
  - [6、所有权与函数](#6所有权与函数)
  - [7、引用](#7引用)
  - [7、可变引用转移所有权了吗？](#7可变引用转移所有权了吗)
  - [8、数组中的引用：切片](#8数组中的引用切片)
  - [从内存上讲引用](#从内存上讲引用)
  - [引用与解引用](#引用与解引用)
  - [所有权和生命周期的关系](#所有权和生命周期的关系)
  - [所有权和引用的关系](#所有权和引用的关系)
  - [4.1.2 所有权规则](#412-所有权规则)
  - [4.1.3 所有权与函数](#413-所有权与函数)
  - [4.2 引用与借用](#42-引用与借用)
  - [4.3 切片](#43-切片)
- [5 Struct](#5-struct)
  - [5.2 struct例子](#52-struct例子)
  - [5.3 struct 方法](#53-struct-方法)
- [6 枚举与模式匹配](#6-枚举与模式匹配)
  - [6.1 定义枚举](#61-定义枚举)
  - [6.2 Option枚举](#62-option枚举)
  - [6.3 match 控制流运算符](#63-match-控制流运算符)
  - [6.4 if let 简单的控制流](#64-if-let-简单的控制流)
- [7 Package、Crate、Module](#7-packagecratemodule)
  - [7.1 代码组织](#71-代码组织)
  - [7.2 Path路径](#72-path路径)
  - [7.3 use](#73-use)
  - [7.4 将模块内容放到其他文件](#74-将模块内容放到其他文件)
  - [8.3 String](#83-string)
    - [1、](#1)
- [10 泛型、Trait 和生命周期](#10-泛型trait-和生命周期)
  - [10.1 提取函数消除重复代码](#101-提取函数消除重复代码)
  - [10.2 泛型数据类型](#102-泛型数据类型)
    - [1、泛型数据类型有什么用？](#1泛型数据类型有什么用)
    - [2、在函数定义中使用泛型](#2在函数定义中使用泛型)
    - [3、在 Struct 定义中使用泛型](#3在-struct-定义中使用泛型)
    - [4、在 Enum 定义中使用泛型](#4在-enum-定义中使用泛型)
    - [5、在方法中使用泛型](#5在方法中使用泛型)
    - [6、泛型参数名称之间的关系](#6泛型参数名称之间的关系)
    - [7、泛型的性能如何呢？](#7泛型的性能如何呢)
  - [10.3 Trait](#103-trait)
    - [1、为什么需要引入 Trait 的概念？](#1为什么需要引入-trait-的概念)
    - [2、Trait 还有什么作用？](#2trait-还有什么作用)
    - [3、如何使用 Trait？](#3如何使用-trait)
    - [4、Trait 默认实现](#4trait-默认实现)
    - [5、Trait 应用场景：Trait Bounds（约束）限制参数类型](#5trait-应用场景trait-bounds约束限制参数类型)
    - [6、Trait 应用场景：Trait Bounds（约束）限制返回类型](#6trait-应用场景trait-bounds约束限制返回类型)
    - [7、Trait 场景：使用 trait 约束来有条件地实现方法](#7trait-场景使用-trait-约束来有条件地实现方法)
  - [10.5 生命周期](#105-生命周期)
    - [1、为什么需要引入生命周期的概念？](#1为什么需要引入生命周期的概念)
    - [2、如何使用生命周期？](#2如何使用生命周期)
    - [3、生命周期标注的类型](#3生命周期标注的类型)
    - [4、生命周期标注的语法](#4生命周期标注的语法)
    - [5、生命周期标记的特性](#5生命周期标记的特性)
    - [6、函数签名中的生命周期标注](#6函数签名中的生命周期标注)
    - [7、函数如何返回一个本地的变量呢？](#7函数如何返回一个本地的变量呢)
    - [8、结构体 Struct 定义中的生命周期标注](#8结构体-struct-定义中的生命周期标注)
    - [9、为什么需要省略生命周期标注（生命周期省略）？](#9为什么需要省略生命周期标注生命周期省略)
    - [10、编译器是如何实现生命周期省略的？](#10编译器是如何实现生命周期省略的)
    - [11、特殊的生命周期 'static](#11特殊的生命周期-static)
  - [19.5 宏 Macro](#195-宏-macro)
    - [1、宏有什么优点和缺点？](#1宏有什么优点和缺点)
    - [2、什么时候使用宏呢？](#2什么时候使用宏呢)
    - [3、函数与宏的区别和联系？](#3函数与宏的区别和联系)
    - [4、什么是声明宏（declarative macro）？](#4什么是声明宏declarative-macro)
    - [5、什么是过程宏（procedural macro）？](#5什么是过程宏procedural-macro)
    - [6、过程宏 1：实现自定义派生（derive）宏](#6过程宏-1实现自定义派生derive宏)
    - [7、过程宏 2：实现属性宏](#7过程宏-2实现属性宏)
    - [8、过程宏 3：实现函数宏](#8过程宏-3实现函数宏)
- [Rust Standard Library Trait 解析](#rust-standard-library-trait-解析)
- [奇怪的小知识](#奇怪的小知识)
- [参考教程](#参考教程)
