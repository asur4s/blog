...
在这篇文章，将会介绍我们是如何构建 Warp 的基础设施：UI 界面，块，输入编辑器。在项目之初，我们设置了一些核心目标：
...
5. **自由的渲染 UI 元素**：为了让 Warp 更易用，我们需要更自由的方式来渲染 UI 元素（消息条，菜单等），而不仅仅像传统终端那样只渲染字符。
6. **直观的编辑体验**：Warp 应支持更直观的编辑体验（如选中文本、光标定位和多光标），这些编辑体验在其他软件都很常见。
