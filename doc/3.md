...
```
> PS：**我发现并没有减少代码行数，只是可以少写 `pub` 关键字**。Trait 中方法的访问控制权限，由 `trait` 前面的关键字决定。


#### 4、Trait 应用场景：Trait Bounds（约束）



Trait 可以和泛型函数搭配使用，要求泛型类型参数具有某种行为。
> **Trait Bounds 为什么能实现**？Rust 中函数是一等公民，将函数签名用来和 Trait 对比，即可实现。
