...
Warp 是一个全新的、高性能的 Terminal，完全由 Rust 编写。命令输入界面（即输入编辑器）同时也是文本编辑器，你可以任意移动光标并选中文字，还可以使用其他文本编辑器中的快捷键。

这篇文章将会介绍我们是如何构建 Warp 的基础设施：UI 界面，块，输入编辑器。在项目之初，我们设置了一些核心目标：
1. **渲染速度**：当终端有大量输出时，渲染速度非常重要。渲染速度不足会导致终端卡顿，这会极大的影响使用体验。故 Warp 应始终在 60FPS 以上，哪怕在 4K 或 8K 的显示器上。
2. **与已存在的 Shell 兼容**：Warp 应兼容目前的主流 Shell。如：Bash、ZSH、Fish。
3. **跨平台**：Warp 应支持多种平台（Mac、Linux、Windows），并支持使用浏览器来协同办公。
4. **和 Shell 集成并支持块特性（包括通过 SSH 连接）**：支持块特性，Warp 需要与 Shell 中正在运行的 Session 深度整合。
5. **自由的渲染 UI 元素**：为了让 Warp 更易用，我们需要采用更自由的方式来渲染 UI 元素（消息条，菜单等），而不仅仅像传统终端那样只渲染字符。
6. **直观的编辑体验**：Warp 应支持更直观的编辑方式（如选中文本、光标定位和多光标）——类似文本编辑器。
...
> 选题：[Asura](Asura)
>
> 原文链接: https://www.warp.dev/blog/how-warp-works