...
下划线 `_` 是 Rust 中的保留标识符。不同的上下文有不同的用途，通常意味着某些东西被忽略。可以用于丢弃变量值，还可以用于表示任意（在 match 部分会讲到）。
```rust
// 42 是一个常量，_ 不会被赋值。
let _ = 42;
...
变量可以用下划线开头，它的作用的告诉编译器，该变量即使未被使用也不要警告。
```rust
let _x = 42;
```
...
```
这种方法叫做结构体更新语法，只能放在末尾部分，并且后面不能由逗号。
...
```
在上一个例子中，变量 name 不能作为静态字符串的引用（即 `&'static str`）。变量 name 是被动态分配的，会被自动释放。它的生命周期是短于整个程序的生命周期。
> 译者注：注意区分 `String` 类型和 `str` 类型。只有 `str` 类型才能作为静态字符串的引用，`format!` 宏的返回值是 `String` 类型。
...
```
<!-- todo -->
在 Rust 中，许多类型都有拥有所有权或引用的变体。
- Strings：`String` 类型拥有所有权，`&str` 是引用。
- Path：`PathBuf` 类型拥有所有权，`&Path` 是引用。
- Collections：`Vec<T>` 类型拥有所有权，`&[T]` 是引用。
...
// v2 = [3, 4]
```
上面不是魔法。`Index` 和 `IndexMut` 这两个 Trait 重载索引操作符 `foo[index]`。

`..` 语法是 Range 的字面量。在标准库中，只有少量的结构可以直接使用 Range。

他们是半闭半开的，当然也可以使用 `=` 符号来让右侧闭合。
```rust
fn main() {
    // 大于 0 的所有数字
...
如果你希望在执行错误后 panic，你可以使用 `.unwrap()`：
```rust
fn main() {
    ...
    // 译：线程 main 发生异常，原因为在 `Err` 类型上调用 `Result::unwrap()` 函数：Utf8Error { valid_up_to: 0, error_len: Some(1) }', src/libcore/result.rs:1165:5
}
```
使用 `.expect()` 可以自定义消息：
```rust
...
`*` 操作符可以用于解构引用，但当你访问字段或者调用方法时不需要手动标注：
```rust
struct Point {
    x: f64,
    y: f64,
...
// 输出： (1, 3)
```
只有类型实现了 Copy Trait，才能像下面这样使用，否则就会报错：
```rust
struct Point {
    x: f64,
    y: f64,
...
fn main() {
    ...
    // 译：错误：不能移出共享引用后面的 *p_ref
}
```
实现 Copy Trait 后即可正常运行：
```rust
// 实现 Copy Trait
#[derive(Clone, Copy)]
struct Point {
...
闭包函数有三种类型，他们分别是 `Fn`、`FnMut`、`FnOnce`。
...
```rust
fn for_each_planet<F>(f: F)
    where F: Fn(&'static str) + 'static // F 必须是静态生命周期。
{
    f("Earth");
    f("Mars");
    f("Jupiter");
...
fn main() {
    ...
    // 译：错误，闭包比当前函数活得更久，但闭包借用了变量 greeting，当前函数拥有变量 greeting 的控制权。 
}
```
稍微修改一下就能正常运行了：
```rust
fn main() {
    ...
    // 变量 greeting 不是被借用到闭包中，而是被移动到闭包中，闭包拥有变量 greeting 的所有权。
}
```
`FnMut` 函数在被调用时，需要可变的借用参数，因此它只能被调用一次。
...
// 输出：8
```
某些闭包需要可变的借用本地变量，就需要用到 `FnMut` 类型。
```rust
fn foobar<F>(mut f: F)
    where F: FnMut(i32) -> i32
{
...
fn main() {
    ...
    foobar(|x| {
        ...
        // 译：错误：在 Fn 类型的闭包中，不能给变量 acc 赋值。编译器建议让 foobar 函数允许接受 FnMut 类型的闭包。
        x * acc
    });
}
```
`FnOnce` 类型的闭包只能被调用一次。某些闭包将本地变量移动到闭包中，运行结束后再将本地变量移动到调用函数，这通常会使用到 `FnOnce` 类型的闭包。
```rust
fn foobar<F>(f: F)
...
fn main() {
    ...
    foobar(move || s);
    // 变量 s 被移动到闭包中，闭包又将变量 s 移动到调用函数。注意 String 类型没有实现 Copy Trait
    // todo
}
```
`FnOnce` 闭包需要被移动。
    ...
    where F: FnOnce() -> String
{
    ...
    // 错误：使用已经移动的值：f
```
你可以用下面的代码来确定闭包已经移动了变量 s，下面的代码是错误的：
```rust
fn main() {
    let s = String::from("alright");
    foobar(move || s);
...
任何可迭代的变量都可以使用 `for in` 来遍历，我们刚看过了 `range` 的遍历，事实上 `Vec` 也可以使用 `for in` 遍历：
```rust
fn main() {
...
// Give me a t
```
即使可迭代的项目使用 filter 过滤或是使用 flatten 遍历，依然可以使用 `for in`：
```rust
fn main() {
...
fn main() {
    // 你可以使用 .into()  来强制类型转换，这里将 &'static str 转换为 String 类型
    let test = make_tester("hunter2".into());
    println!("{}", test("******"));
    println!("{}", test("hunter2"));
}
```
你甚至可以移动一个引用来作为函数的参数作为返回的闭包的生命周期：
```rust
